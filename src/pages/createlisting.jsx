...
export default function Createlisting() {
    ...
    const { type, name, bedrooms, bathrooms, Parking, furnished, Address, Description, offer, regular, discount, images } = FormData;
    ...
    async function onSubmit(e) {
        ...
        if (offer && discount <= 0) {
            setLoading(false);
            toast.error("Discounted price should be greater than 0.");
            return;
        }
        ...
        async function storageImage(image) {
            return new Promise((resolve, rejected) => {
                ...
                const filename = `${auth.currentUser.uid}-${image.name}-${Math.floor(Math.random() * 100)}`;
        ...
        try {
            const imgUrl = await Promise.all([...images].map((image) => storageImage(image)));
            const formDataCopy = {
                ...FormData,
                images,
